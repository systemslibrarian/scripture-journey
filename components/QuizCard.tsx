...
type QuizCardProps = {
  quiz: QuizData
  lessonSlug?: string
  onScore?: (result: { multipleChoice: boolean; fillInBlank?: boolean }) => void
}

export default function QuizCard({ quiz, lessonSlug, onScore }: QuizCardProps) {
  const { question, choices, answer, fillInBlank, explanation } = quiz
  ...
  function emitScore(result: { multipleChoice: boolean; fillInBlank?: boolean }) {
    onScore?.(result)
    if (lessonSlug) {
      saveQuizScore(lessonSlug, result)
      window.dispatchEvent(new Event('quiz-score-saved'))
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 rounded-2xl border border-[#e6dccd] bg-[#fffdf8] p-4">
        ...
        <h2 className="mt-2 text-lg font-bold text-[#1b1a17]" id="mc-question">
          {question}
        </h2>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          <button
            ...
            onClick={() => {
              ...
              const mcCorrect = selectedChoice === answer
              if (!fillInBlank) {
                emitScore({ multipleChoice: mcCorrect })
              }
            }}
          ...
          {choiceSubmitted && !isChoiceCorrect && explanation && (
            <div
              className="mt-2 w-full rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900"
              role="note"
            >
      ...
      {fillInBlank && (
        <div className="mt-4 rounded-2xl border border-[#e6dccd] bg-[#fffdf8] p-4">
          ...
          <label htmlFor="fill-in-blank" className="mt-2 block text-sm font-semibold text-[#1b1a17]">
            {fillInBlank.prompt}
          </label>
          ...
          <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center">
            <input
              ...
              aria-describedby={blankSubmitted ? 'blank-feedback' : undefined}
            ...
            <button
              ...
              onClick={() => {
                ...
                const fibCorrect = acceptedBlankAnswers
                  .map((c) => normalizeAnswer(c))
                  .includes(normalizeAnswer(blankValue))
                emitScore({
                  multipleChoice: selectedChoice === answer,
                  fillInBlank: fibCorrect,
                })
              }}
